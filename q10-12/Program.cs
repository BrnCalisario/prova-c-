...
public class Pesquisador
{
    ...
    {
        var query = uni.Disciplinas.Where(d => d.Nome.Length > 10);
    ...
    {
        var queryAlpha = uni.Departamentos.OrderBy(d => d.Nome);

        var disciplinas =
            uni.Disciplinas
            .GroupBy(di => di.DepartamentoID)
            .Select(group => new
            {
                ID = group.Key,
                count = group.Count()
        ...
        var queryFinal = queryAlpha.Join(
            disciplinas,
            dep => dep.ID,
            dis => dis.ID,
            (dep, dis) => new
            {
                ...
                qtdDisciplina = dis.count
            }
        );

        foreach (var d in queryFinal)
            WriteLine($"Departamento: {d.nomeDepartamento}, número de disciplinas: {d.qtdDisciplina}");
    }
    ...
    {
        var queryA = uni.Alunos;


        // var professores = queryA.Select(a => a.TurmasMatriculados.Join(uni.Turmas,
        //     tM => tM,
        //     turma => turma.ID,
        //     (tM, turma) => new{
        //         professores = uni.Professores.Where(p => p.ID == turma.ProfessorID)
        //     }));

        var query = uni.Alunos.Select(a => a.TurmasMatriculados);
        ...
        var professorResponsavel = 
            uni.Turmas
                ...
                .Select(group => new {
                    id = group.Key,
                    professor = uni.Professores.Where(p => p.ID == group.Key).First()
                });

        var queryAluno = uni.Alunos.Select(a => new {
            nomeAluno = a.Nome,
            idadeAluno = a.Idade,
            professores = a.TurmasMatriculados.OrderBy(t => t)
                .Select(t => professorResponsavel
                .Where(p => p.id == t)
                .Select(p => new {
        ...
        foreach(var a in queryAluno)
        {
            WriteLine($"Nome {a.nomeAluno}, Idade {a.idadeAluno} anos ");
            WriteLine("Professores: ");
            foreach(var p in a.professores)
                p.ToList().ForEach(p => Write(p.nomeProfessor + ", "));
            WriteLine();
        }
    ...
    {
        var queryTurma = uni.Alunos.Select(a => new{
            turmas = a.TurmasMatriculados.Select(t => new{
                nomeTurma = uni.Turmas.Where(turma => t == turma.ID).First().Codigo,
                idProf = uni.Turmas.Where(turma => t == turma.ID).First().ProfessorID,
            }).First()
        });

        var queryProfessor = uni.Professores.Select(p => new {
            nomeProfessor = p.Nome,
            qntAlunos = queryTurma.Where(qt => qt.turmas.idProf == p.ID).Count(),
        });

        foreach(var i in queryProfessor)
        {
            WriteLine("Nome do professor: " + i.nomeProfessor);
            WriteLine("Quantidade de alunos: " + i.qntAlunos);
